'''
Создать (не программно) текстовый файл со следующим содержимым:
One — 1
Two — 2
Three — 3
Four — 4
Необходимо написать программу, открывающую файл на чтение
и считывающую построчно данные. При этом английские
числительные должны заменяться на русские.
Новый блок строк должен записываться в новый текстовый файл.
'''

file_name = 'test_file4.txt'
new_file_name = 'test_file4_new.txt'

rus_names = {'One' : 'Один', 'Two' : 'Два', 'Three' : 'Три', 'Four' : 'Четыре'}
new_file = []
with open(file_name, 'r') as file_obj:

    for i in file_obj:
        i = i.split(' ', 1)
        new_file.append(rus_names[i[0]] + '  ' + i[1])
    print(new_file)

with open(new_file_name, 'w') as file_obj_2:
    file_obj_2.writelines(new_file)
